    ...
    public static class FlashHelpers {
        ...
        public static string Flash(this HtmlHelper helper) {
            ...
            if (!String.IsNullOrEmpty(message)) {
                ...
                if(className!="error")
                {
                    sb.AppendLine("$('#flash .ui-state-error').removeClass('ui-state-error').addClass('ui-state-highlight');");
                    sb.AppendLine("$('#flash .ui-icon-alert').removeClass('ui-icon-alert').addClass('ui-icon-info');");
                }
                sb.AppendFormat("$('#flash .flashMsg').html('{0}');", HttpUtility.HtmlAttributeEncode(message).Replace("\"", "").Replace("'", "").Replace('\n', ' ').Replace('\f', ' ').Replace('\r', ' '));
                sb.AppendFormat("$('#flash .flashType').html('{0}: ');", className);
