...
<%foreach (var bonus in Model.BonusList){%>
    <tr class="<%= lineIndex%2==0 ? "ui-state-default": "odd"%>">
        <td class="l"><%=Html.Hidden("bonusBet[" + index + "].BonusBetId", bonus.BonusBetId)%><%=Html.Hidden("bonusBet[" + index + "].Bonus", bonus.Bonus.BonusId)%><%=bonus.Bonus.Name%></td>
        <td class="l"><%=Html.DropDownList("bonusBet[" + index + "].Team", bonus.Teams, "--- Selecione ---")%></td>   
