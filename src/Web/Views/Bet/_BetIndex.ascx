...
<%foreach (var date in Model.BetList.GroupBy(x => new DateTime(x.Match.StartTime.Year, x.Match.StartTime.Month, x.Match.StartTime.Day)).Select(x => x.Key)){%>
    ...
    <tr class="<%= lineIndex%2==0 ? "ui-state-default": "odd"%>">
        ...
        <td class="r homeTeam"><%: matchBet.Match.Team1Obj.Name%></td>
        <td class="c"><%= Html.TeamFlag(matchBet.Match.Team1)%></td>
        ...
        <td class="c"><%= Html.TeamFlag(matchBet.Match.Team2)%></td>
        <td class="l awayTeam"><%: matchBet.Match.Team2Obj.Name%></td>
