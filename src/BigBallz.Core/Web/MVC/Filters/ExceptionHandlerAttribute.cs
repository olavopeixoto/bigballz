...
namespace BigBallz.Core.Web.MVC.Filters
{
    ...
    public class ExceptionHandlerAttribute : FilterAttribute, IExceptionFilter
    {
            ...
            set
            {
                ...
                if (!typeof(Exception).IsAssignableFrom(value))
                {
                    throw new ArgumentException(String.Format(CultureInfo.CurrentUICulture,
                        "Type {0} is not an exception", value.FullName));
                }
                                                                     ...
                                                                     new
                                                                         {
                                                                             ...
                                                                             aspxerrorpath = filterContext.RequestContext.HttpContext.Request.Path
                                                                         }));
            else
            {
                if (filterContext.HttpContext.Request.IsAjaxRequest())
                {
                    filterContext.Result = new JsonResult
                                               {
                                                   Data = RenderAjaxErrorMessage(filterContext.Exception)
                                               };
                }
                else
                {
        ...
        private static string RenderAjaxErrorMessage(Exception exception)
        {
